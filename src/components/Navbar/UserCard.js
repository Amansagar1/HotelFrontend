...
const UserCard = ({ isOpen }) => {
  ...
  useEffect(() => {
    ...
    if (userFullName && userEmail) {
      setUserInfo({ username: userFullName, email: userEmail });
    } else if (session && session.user) {
      setUserInfo({ name: session.user.username, email: session.user.email });
    }
  }, [session]);
        ...
        {userInfo.image ? (
          ...
          <div className="w-24 h-24 flex justify-center items-center rounded-full bg-gray-300 border-4 border-gray-300">
            <span className="text-white text-lg font-semibold">
              {getInitials(userInfo.username, userInfo.email)} {/* Pass both name and email */}
            </span>
          </div>
        )}
      </div>
